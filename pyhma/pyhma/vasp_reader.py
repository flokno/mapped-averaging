...
def read(
    vasprun_files, force_tol=0.001, raw_files=False, fermi_dirac=False, verbose=False
):
    """
  ...
  * The read() function handles incomplete XML file(s) generated from interrupted AIMD runs (by the user, or due to some time constraint). This was possible by using the recover capability of the LXML parser.
  ...
  """
    ...
    parser = lxml.etree.XMLParser(
        recover=True
    )  # LXML parser with the capability to handle broken (incomplete) XML files
    ...
    for i, vasprun_file_i in enumerate(vasprun_files):
        tree = lxml.etree.parse(
            vasprun_file_i, parser
        )  # parsing the whole vasprun.xml file
        if verbose:
            if i == 0:
        ...
        if i == 0:
            ...
            volume_atom = (
                float(tree.find("./structure/crystal/i[@name='volume']").text)
                / num_atoms
            ...
            for v in tree.find(
                "./structure[@name='initialpos']/varray[@name='positions']"
            ):
            ...
            if verbose:
                ...
                for l in range(len(force_0)):
                    print(
                        " %3d  %12.8f %12.8f %12.8f    %12.8f %12.8f %12.8f"
                        % (l + 1, *basis[l], *force_0[l])
                    )
                print("")
        ...
        if i == (0 if n_files == 1 else 1):
            ...
            if fermi_dirac and ismear == -1:
                ...
                print(
                    " It CAN NOT be used to compute anharmonic free energy using thermodynamic integration from T = 0 K,"
                )
                print(
                    " because the potential energy surface is T-dependent in this case."
                )
                print(
                    " It can ONLY be used if interested in a free energy derivative property (e.g., energy or pressure)."
                )
                print(
                    " In this case, electronic contribution (due to thermal excitation) is included in the measured property.\n"
                )
            ...
            if fermi_dirac != (ismear == -1):
                ...
                print(" ======")
                print(
                    " Inconsistent smearing methods: ISMEAR=%s and fermi_dirac=%s."
                    % (ismear, fermi_dirac)
                )
                if ismear == -1:
                    print(
                        " Note that Fermi-Dirac ststistics (ISMEAR=-1) can not be used directly to compute anharmonic free energy"
                    )
                    print(
                        " using thermodynamic integration from T = 0 K. Ground-state DFT must be used (e.g., ISMEAR=1)."
                    )
                print(" EXITING pyHMA!\n")
                return

            timestep = float(
                tree.find("./incar/i[@name='POTIM']").text
            )  # timestep (fs)
            temperature = float(
                tree.find("./incar/i[@name='TEBEG']").text
            )  # temperature (K)
            ...
            pressure_ig = (
                kBT_J / (volume_atom * 1e-30) * 1e-9
            )  # ideal gas pressure (GPa)
    ...
    if raw_files:
        _make_raw_files(
            num_atoms, box_row_vecs, basis, position, force, energy, pressure
        )

    # return a dict of the extracted data
    return {
        "box_row_vecs": box_row_vecs,
        "num_atoms": num_atoms,
        "volume_atom": volume_atom,
        "basis": basis,
        "position": position,
        "force": force,
        "energy": energy,
        "pressure": pressure,
        "pressure_ig": pressure_ig,
        "timestep": timestep,
  ...
  """
    ...
    for i, f in enumerate(force):
        ...
        if force_mag > force_tol:
            print(
                " WARNING! Magnitude of force on atom ",
                (i + 1),
                " is ",
                force_mag,
                " (> force_tol = ",
...
def _make_raw_files(num_atoms, box_row_vecs, basis, position, force, energy, pressure):
  ...
  """
    ...
    file_poscar_eq = open(
        "poscar_eq.dat", "w"
    )  # initial POSCAR file (in fractional coordinates)
