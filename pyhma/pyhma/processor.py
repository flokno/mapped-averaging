...
class Processor:
  ...
  """
    ...
    def __init__(self, data, pressure_qh, meV=False):
        ...
        self.basis = np.array(
            data["basis"]
        )  # atomic positions of initial configuration (fractional)
        self.position = np.array(
            data["position"]
        )  # positions at each atom at each MD step (fractional)
        self.force = np.array(
            data["force"]
        )  # forces at each atom at each MD step (eV/A)
        self.energy = np.array(
            data["energy"]
        )  # instantaneous potential energy (eV/atom)
        ...
        self.out_data = np.empty(
            (0, 4)
        )  # anharmonic data array ([e_ah_conv, e_ah_hma, p_ah_conv, p_ah_hma])
        self.meV = meV
    ...
    """
        ...
        if steps_tot == None:
            ...
            if steps_tot > len(self.energy):
                print(
                    "\n WARNING! User-set steps_tot (",
                    steps_tot,
                    ") can not be larger than MD simulation steps (",
                    len(self.energy),
                    ").",
                )
        ...
        f_v = (self.pressure_qh - self.pressure_ig) / (
            3 * (self.num_atoms - 1) * kBT_J
        )  # f_v variable HMA pressure (GPa/J)
        ...
        if verbose:
            ...
            if self.ismear == -1:
                print(
                    " Electronic free energy (F=E-TS) calculations using Fermi-Dirac statsitics (ISMEAR=-1)"
                )
            else:
                print(
                    " Ground-state DFT calculations (E0) using ISMEAR=%s"
                    % (self.ismear)
                )
            ...
            print(
                " Harmonic energy (eV/atom): %10.5f"
                % (1.5 * kBT_eV * (self.num_atoms - 1) / self.num_atoms)
            )
        ...
        if self.meV:
            e_fac = 1.0e3
        with open("energy_ah.out", "w") as file_energy_ah, open(
            "pressure_ah.out", "w"
        ) as file_pressure_ah:
            ...
            for step in range(self.steps_tot):  # snaps
                ...
                # HMA
                r_cart = Processor._direct_to_cart(
                    self.position[step], self.box_row_vecs
                )
                fdr = 0
                ...
                e_ah_hma = e_fac * (
                    self.energy[step] + 0.5 * fdr / self.num_atoms - energy_lat
                )
                p_ah_hma = (
                    self.pressure[step]
                    - self.pressure_ig
                ...
                print(
                    "%10.1f  %10.5f  %10.5f" % (sim_time, e_ah_conv, e_ah_hma),
                    file=file_energy_ah,
                )
                print(
                    "%10.1f  %10.5f  %10.5f" % (sim_time, p_ah_conv, p_ah_hma),
                    file=file_pressure_ah,
                )
                self.out_data = np.append(
                    self.out_data, [[e_ah_conv, e_ah_hma, p_ah_conv, p_ah_hma]], axis=0
                )
    ...
    """
        ...
        if self.steps_tot < steps_eq:
            print(
                "WARNING! Number of equilibaration steps (",
                steps_eq,
                ") can not be larger than total steps (",
                self.steps_tot,
                ").",
            )
            ...
            print(
                "WARNING! Number of blocks ((steps_tot-steps_eq)/blocksize) must be at least two."
            )
            print(
                "         Reduce blocksize to get finite number of blocks and try again."
            )
        ...
        if verbose:
            ...
            print("==========================")
            print(
                "", n_prod, "production steps (after", steps_eq, "equilibration steps)"
            )
            print(
                "",
                int((self.steps_tot - steps_eq) / blocksize),
                "blocks (blocksize =",
                blocksize,
                " steps)\n",
            )
        ...
        data_avg = np.average(
            data_prod, axis=0
        )  # used raw data because average should be independent on blocks
        # Uncertainty
        data_err = np.std(data_prod_block, ddof=1, axis=0) / np.sqrt(
            len(data_prod_block)
        )
        # Correlation
        ...
        self.stats = {
            "e_ah_conv": {"avg": data_avg[0], "err": data_err[0], "cor": data_cor[0]},
    ...
    """
        ...
        print(
            "\n e_ah_conv (%s/atom): %10.5f +/- %5.1e    cor: %4.2f"
            % (
                e_units,
                stats["e_ah_conv"]["avg"],
                stats["e_ah_conv"]["err"],
        ...
        print(
            " e_ah_hma  (%s/atom): %10.5f +/- %5.1e    cor: %4.2f"
            % (
                e_units,
                stats["e_ah_hma"]["avg"],
                stats["e_ah_hma"]["err"],
        ...
        print(
            " p_ah_conv      (GPa): %10.5f +/- %5.1e    cor: %4.2f"
            % (
                stats["p_ah_conv"]["avg"],
                stats["p_ah_conv"]["err"],
                stats["p_ah_conv"]["cor"],
        ...
        print(
            " p_ah_hma       (GPa): %10.5f +/- %5.1e    cor: %4.2f\n"
            % (
                stats["p_ah_hma"]["avg"],
                stats["p_ah_hma"]["err"],
                stats["p_ah_hma"]["cor"],
    ...
    def _get_cor(
        data,
    ):  # without the N-1 correction in both numerators and denominators
