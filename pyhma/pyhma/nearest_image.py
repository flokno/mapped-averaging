...
class NearestImage:
  ...
  """
    ...
    """
        ...
        while i != lastTransform:
            ...
            if abs(dot) > self.halfTol:
                dot = round(dot)
                dr -= (
                    dot * self.transform_vecs[i]
                )  # MUST do -= to keep the reference/pointer! If you do dr = dr + x, yu only change value and get new reference
    ...
    """
        ...
        tmp2 = 0.5 * v
        for i in range(len(self.transform_vecs)):
            dot = abs(tmp2.dot(self.transform_vecs[i])) / (
                self.transform_vecs[i].dot(self.transform_vecs[i])
            )
